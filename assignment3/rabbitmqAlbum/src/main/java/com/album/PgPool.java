...
//        Class.forName("com.mysql.cj.jdbc.Driver");
    ...
    public long save(AlbumCollection collection) throws SQLException, ClassNotFoundException, IOException {
        ...
        try (Connection conn = getConnection()){
            PreparedStatement ps = conn.prepareStatement("select id, like_count, dislike_count from album_like where id = " + id, ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            ResultSet rs = ps.executeQuery();
            if(rs.first()){
                long likeCount = rs.getLong("like_count");
                long disLikeCount = rs.getLong("dislike_count");
                ps = conn.prepareStatement("update album_like set like_count = ?, dislike_count = ? where id = ?");
                ps.setLong(1, likeCount + collection.getLikeCount());
                ps.setLong(2, disLikeCount + collection.getDislikeCount());
                ps.setLong(3, id);
            }else{
