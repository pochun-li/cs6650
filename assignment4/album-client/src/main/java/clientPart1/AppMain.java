...
public class AppMain {
  ...
  public static void request(String ipAddr, CountDownLatch latch){
    try (CloseableHttpResponse response = HttpUtil.get(ipAddr + "/review/" + random.nextInt(10))){
...
//          System.out.println("GET:" + EntityUtils.toString(response.getEntity()));
  ...
  public static void request(int num, String ipAddr, CountDownLatch latch){
    ...
    for (int i = 0; i < num; i++) {
//      try (CloseableHttpResponse response = HttpUtil.get(ipAddr + "/album/1")){
//        if (response.getCode() == 200){
//          System.out.println("GET:" + EntityUtils.toString(response.getEntity()));
//          AppMain.SUCCESS.incrementAndGet();
//        }
...
//      }
      long albumId = -1;
      try (CloseableHttpResponse response = HttpUtil.post(ipAddr + "/album/", body)){
        if (response.getCode() == 200){
...
//          System.out.println("POST:" + EntityUtils.toString(response.getEntity()));
      ...
      if(albumId > 0){
        try (CloseableHttpResponse response = HttpUtil.post(ipAddr + "/review/" + albumId + "/like", body)){
          if (response.getCode() == 200){
//          System.out.println("GET:" + EntityUtils.toString(response.getEntity()));
            AppMain.SUCCESS.incrementAndGet();
          }
...
//          System.out.println("GET:" + EntityUtils.toString(response.getEntity()));
        ...
        try (CloseableHttpResponse response = HttpUtil.post(ipAddr + "/review/" + albumId + "/dislike", body)){
          if (response.getCode() == 200){
//          System.out.println("GET:" + EntityUtils.toString(response.getEntity()));
            AppMain.SUCCESS.incrementAndGet();
          }
