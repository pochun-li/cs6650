...
//        Class.forName("com.mysql.cj.jdbc.Driver");
     ...
     * @throws ClassNotFoundException
     */
    public AlbumCollection countById(Long id) throws SQLException, ClassNotFoundException, IOException {
        try (Connection conn = getConnection()){
            AlbumCollection collection = new AlbumCollection();
            PreparedStatement ps = conn.prepareStatement("select like_count, dislike_count from album_like where id = " + id);
            ResultSet rs = ps.executeQuery();
            if(rs.next()){
                collection.setId(rs.getLong("id"));
                collection.setLikeCount(rs.getLong("like_count"));
                collection.setDislikeCount(rs.getLong("dislike_count"));
                return collection;
            }
        }
        return null;
